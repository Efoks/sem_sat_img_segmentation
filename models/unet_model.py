...
from src.models import prepare_u_net

torch.cuda.empty_cache()

config.print_model_config(config.unet)

def launch_model():

    experiment = '_exp_1_tversky_loss'

    num_epochs = config.unet['num_epochs']
    # num_epochs = 3
    ...
    (supervised_loader_train,
     ...
     unsupervised_loader_val) = dh.create_data_loaders(batch_size,
                                                        perform_stratiication= False,
                                                        model='unet')

    model = prepare_u_net()

    experiment_dir = utils.create_experiment_dir(exp_name=experiment, model_name='unet_' + config.unet['backbone'])
    ...
    torch.save(model.state_dict(), os.path.join(config.UNET_SAVE_DIR,
                                                f'unet_{config.unet["backbone"]}_{experiment}_state_dict.pth'))
