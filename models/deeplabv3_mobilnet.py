...
from src.models import prepare_deeplabv3_mobilenet

torch.cuda.empty_cache()

config.print_model_config(config.mobilenet)

def launch_model():

    experiment = '_exp_1_tversky_loss'

    num_epochs = config.mobilenet['num_epochs']
    # num_epochs = 3
    batch_size = config.mobilenet['batch_size']
    ...
    model = prepare_deeplabv3_mobilenet()

    experiment_dir = utils.create_experiment_dir(exp_name=experiment, model_name='deeplabv3_' + config.mobilenet['backbone'])
    ...
    torch.save(model.state_dict(), os.path.join(config.DEEPLABV3_MOBILENET_SAVE_DIR,
                                                f'deeplabv3_{config.mobilenet["backbone"]}_{experiment}_state_dict.pth'))
